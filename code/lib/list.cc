...
List<T>::SanityCheck() const
{
    ...
    if (this->first == NULL) {
	ASSERT((this->numInList == 0) && (this->last == NULL));
    } else if (this->first == this->last) {
	ASSERT((this->numInList == 1) && (this->last->next == NULL));
    } else {
        for (numFound = 1, ptr = this->first; ptr != this->last; ptr = ptr->next) {
	    numFound++;
            ASSERT(numFound <= this->numInList);	// prevent infinite loop
        }
        ASSERT(numFound == this->numInList);
...
List<T>::SelfTest(T *p, int numEntries)
{
    ...
    ASSERT(this->IsEmpty() && (this->first == NULL));
